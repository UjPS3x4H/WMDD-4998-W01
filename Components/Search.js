...
const Search = () => {
  ...
  const [pickerBorderColor, setPickerBorderColor] = useState(
    pickerSelectStyles.inputAndroid.borderColor
  );

  const [noteText, setNoteText] = useState("Please select a search type.");

  const handleSearch = () => {
    if (!searchQuery) {
      setTextInputBorderColor("red");
      setPickerBorderColor("red");
      setNoteText("Movie/TV show name is required.");
      return;
    } else {
      setTextInputBorderColor(styles.inputContainer.borderColor);
      setPickerBorderColor(pickerSelectStyles.inputAndroid.borderColor);
      setNoteText("Please select a search type.");
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.centeredContainer}>
        <View
          style={[styles.inputContainer, { borderColor: textInputBorderColor }]}
        >
          <Icon
            name="search"
            size={20}
      ...
      {searchInitiated ? (
        ...
        <Text
          style={[
            styles.note,
            { color: textInputBorderColor === "red" ? "red" : "black" },
          ]}
        >
          {noteText}
        </Text>
